...
<script>
    ...
    /*
    $('body').on('click','.text_votos #no-me-gusta',function(e){
        e.preventDefault();
        //$('.text_votos #no-me-gusta fa-thumbs-up').css('color','blue');
        var id_coment = $(this).data('id');
        var slug = $('#slug').val();

        $.ajax({
            url:dislike+'/'+slug+'/'+id_coment,
            type:'get',
            success:function(response){
                //alert(response);
                //$('.text_votos #me-gusta fa-thumbs-up').css('color','blue');
            }
        });
    });
    */
